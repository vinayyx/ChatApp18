...
export default function UserList({ users = [], openChat }) {
  ...
  if (!users || users.length === 0) {
    return <div className="w-full md:w-96 p-4 bg-white shadow-lg rounded-lg">No users found</div>;
  }
          ...
          >
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`w-3 h-3 rounded-full ${isOnline ? "bg-green-500" : "bg-gray-400"} inline-block ml-2`}
                  title={isOnline ? "Online" : "Offline"}
                ></span>
              </div>
              {isOnline && <small className="text-green-600 text-xs">Online</small>}
            </div>
            ...
            <div className="flex flex-col items-end">
              ...
              <button
                onClick={() => openChat(user._id)} // <--- ID use karo, username nahi
                className="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 flex items-center gap-1 mt-1 text-xs md:text-sm"
              >
                Chat
              </button>
            </div>
